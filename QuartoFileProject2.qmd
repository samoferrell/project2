...
```{r}
bones_cast <- search_show(show_name = "Glee", endpoint = "cast")
cast <- bones_cast |>
...
SEASON GRAPHS:
```{r}
ep_ratings <- search_show(show_name = "Game of Thrones", endpoint = "episodes")
eps <- ep_ratings |>
        unnest_wider(rating, names_sep = "_") |>
  select(season, number, rating_average) |>
  group_by(season) %>%                        
  summarise(mean_rating = mean(rating_average, na.rm = TRUE))
seasons <- search_show(show_name = "Game of Thrones", endpoint = "seasons")
season_ratings <- seasons |>
          unnest_wider(image, names_sep = "_") |>
  select(number, image_original) |>
  rename("season" = "number") |>
  left_join(eps, by = "season") |>
  mutate(season = as.factor(season))
```

```{r}
ggplot(season_ratings, aes(x = season, y = mean_rating, fill = season)) +
...
```{r}
ggplot(season_ratings, aes(x = season, y = mean_rating)) +
  geom_bar(stat = "identity") +
  geom_image(aes(image = image_original, y = 0), size = 0.31) + 
  labs(title = "Mean Episode Rating Across Seasons") +
