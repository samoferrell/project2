...
shinyServer(function(input, output, session) {
  search_show <- function(url = "https://api.tvmaze.com/shows", 
                          show_name = NULL, 
                          endpoint = NULL){
    ...
    if (!is.null(show_name) & (!is.null(endpoint))){
      if (!(tolower(show_name) %in% tolower(all_shows$name))){
        stop("That show doesn't appear to be in the data set, try 'Breaking Bad'")
      }
      else {
        subsetted <- subset(all_shows, tolower(name) == tolower(show_name))
        id <- subsetted$id
    ...
    else if (is.null(endpoint) & (!is.null(show_name))){
      if (!(tolower(show_name) %in% tolower(all_shows$name))){
        stop("That show doesn't appear to be in the data set, try 'Breaking Bad'")
      }
      else{
